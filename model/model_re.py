...
class ImagePatchEmbed(nn.Module):
    ...
    def forward(self, image):
        ...
        image = self.conv_layer(image).flatten(2).transpose(1, 2) 
        return image
...
class Text2ImageGate(nn.Module):
    ...
    def forward(self, text):
        text = text * torch.view_as_complex(self.select_para)  
        text = text.permute(0, 2, 1)
        text = self.avg_pool(text.real)  
